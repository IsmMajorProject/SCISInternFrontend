...
function Applications() {
  ...
  const getApplications = async function () {
    try {
      let applications = await axios.get(
        `${apiUrl.backend}/api/application/bylisting/${listingId}`
      );
      applications = applications.data.applications;
      console.log(applications);

      let applicants = await axios.get(`${apiUrl.backend}/api/applicant/bylisting/${listingId}`);
  ...
  const updateApplication = (applicationId, newStatus) => {
    let url = `${apiUrl.backend}/api/application/${applicationId}`;
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.name}>
                    ...
                    <TableCell align="center">{actionButtons(row)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
